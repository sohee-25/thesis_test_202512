...
## 10. 이미지 포함 예시 (복붙 그대로 사용)

아래 예시처럼 README 내 원하는 위치에 이미지 삽입:

```
### 그래프 차수 분포 (baseline)
![degree](baseline/graph_degree_hist.png)

### SVD 공격 구간 anomaly top 리스트
![svd-top](attack_1058085_1101285/top_anomalies.png)

### Node2Vec 공격 구간 anomaly top 리스트
![n2v-top](node2vec_experiment/attack_1166400_1209600/top_anomalies.png)
```
...
python experiment.py \
  --auth C:/Datasets/NANL/auth.txt.gz \
  --red  C:/Datasets/NANL/redteam.txt.gz \
  --out  ./lanl_final_experiment/
```

- baseline + 모든 redteam 시간대 자동 탐지  
- 각 window별 그래프 생성, 임베딩, 이상 점수, 시각화 저장
...
이 레포는  
“LANL 데이터 기반 컴퓨터–컴퓨터 그래프 + 사용자 행동 분석”을  
일관된 실험 단위로 구성하여,  
논문 실험 계획에서 요구되는 **정합성·재현성·설득력**을 갖추도록 설계됨.
